...
async def gathered_tasks(link, query_id, total_reviews):
    # url to be scraped
...
async def main_scraper_func():
    table_data = pd.read_sql(
        "SELECT q.query_id,q.business_link,hd.english_review FROM query q INNER JOIN hotel_data hd ON q.query_id = hd.query_id WHERE q.review_data=0", db)
    await asyncio.gather(*[gathered_tasks(table_data['business_link'][i], table_data['query_id'][i], table_data['english_review'][i]) for i in range(len(table_data['business_link']))])
